...
\indent Baigiamajame bakalauro darbe nagrinėjama Android vietinių bibliotekų apsauga naudojant vykdymo meto iššifravimą ir vietinių funkcijų perėmimo mechanizmus. Teorinėje dalyje analizuojami Android vietinių bibliotekų įkėlimo mechanizmai, ELF formato dinaminio susiejimo struktūros ir perėmimo technikos: tiesioginis instrukcijų pakeitimas, PLT/GOT lentelių modifikavimas, bibliotekų interpozicija ir įkėlimo taško perėmimas. Lyginami Android perėmimo karkasai Frida, LSPosed ir ByteHook pagal tinkamumą vietinių bibliotekų įkėlimo perėmimui be sistemos privilegijų. Praktinėje dalyje suprojektuotas ir įgyvendintas konceptualus apsaugos sprendimas naudojant PLT pagrįstą perėmimą per ByteHook biblioteką. Sistema kompiliavimo metu užšifruoja pasirinktas vietines bibliotekas XOR šifru, o vykdymo metu perima \texttt{\_\_loader\_dlopen} funkcijos iškvietimus, iššifruoja apsaugotas bibliotekas atmintyje per \texttt{memfd\_create} ir įkelia jas naudojant \texttt{android\_dlopen\_ext} sąsają. Sprendimas ištestuotas Google Pixel~9 įrenginyje su Android~16 (API lygis~36). Atlikti 15 sisteminių testų, patvirtinančių teisingą perėmimą, šaltojo paleidimo pridėtines sąnaudas mažesnes nei 15\,ms ir praeinančio iškvietimo sąnaudas mažesnes nei 50\,$\mu$s. Rezultatai patvirtino, kad PLT pagrįstas perėmimas gali skaidriai apdoroti vietinių bibliotekų tarpusavio įkėlimo scenarijus, kurių neaptarnauja Java lygio mechanizmai.\par
\vspace{0.4em}
\setlength{\parindent}{0pt}
Darbo sudaro 6 dalys: įvadas, aktualių technologijų analizė ir apžvalga, sistemos specifikacija, sistemos įgyvendinimas, sistemos testavimas ir išvados.\par
\vspace{0.2em}
% NOTE: verify page count after compilation and adjust if needed
Darbo apimtis\,--\,45 p.\ teksto su priedais, 13 iliustr., 7 lent., 16 bibliografinių šaltinių.\strut
}}
...
\noindent
\fbox{\parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule}{%
\strut\textbf{Prasminiai žodžiai:} Android, vietinės bibliotekos, PLT perėmimas, šifravimas, memfd, dinaminis susiejimas, ByteHook.\strut
}}
...
\setlength{\parindent}{1.5em}
\indent This bachelor's thesis examines the protection of Android native libraries using runtime decryption and native call hooking mechanisms. The theoretical part analyzes Android native library loading mechanisms, ELF dynamic linking structures, and interception techniques including inline patching, PLT/GOT table modification, library interposition, and loader entry point interception. Existing Android hooking frameworks, Frida, LSPosed, and ByteHook, are compared for their suitability for intercepting native library loading without requiring system privileges. In the practical part, a proof-of-concept protection solution was designed and implemented using PLT-based interception through the ByteHook library. The system encrypts selected native libraries with an XOR cipher at build time and intercepts \texttt{\_\_loader\_dlopen} function calls at runtime, decrypts protected libraries in memory using \texttt{memfd\_create}, and loads them through the \texttt{android\_dlopen\_ext} interface. The solution was tested on a Google Pixel~9 device running Android~16 (API level~36). Fifteen system tests confirmed correct interception behaviour, cold start overhead below 15\,ms, and per-call passthrough overhead below 50\,$\mu$s. The results demonstrated that PLT-based interception can transparently handle native-to-native library loading scenarios that are not addressed by Java-level hooking mechanisms.\par
\vspace{0.4em}
\setlength{\parindent}{0pt}
The thesis consists of 6 parts: introduction, analysis of relevant technologies, system specification, system implementation, system testing, and conclusions.\par
\vspace{0.2em}
% NOTE: verify page count after compilation and adjust if needed
The thesis consists of 45 pages of text with appendices, 13 figures, 7 tables, and 16 bibliographic sources.\strut
}}
...
\noindent
\fbox{\parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule}{%
\strut\textbf{Keywords:} Android, native libraries, PLT hooking, encryption, memfd, dynamic linking, ByteHook.\strut
}}
...
\setcounter{page}{5}  % Pages: title(1), approval(2), task(3), annotation LT(4), annotation EN(5), TOC starts here
...
The protection system is designed to resist a static adversary: an attacker who has obtained the APK file and attempts to extract and reverse-engineer the native libraries it contains using tools such as disassemblers, hex editors, or ELF analysis utilities. Against this adversary, the system ensures that no valid, loadable version of a protected library exists on persistent storage, so the extracted file cannot be analyzed or reused without the decryption key. The XOR cipher used in this proof of concept is intentionally simple and could be reversed by a determined attacker through brute force. The contribution of this work is the interception mechanism rather than the strength of the cipher, and the hooking layer is cipher-agnostic, meaning any stronger cipher could replace the XOR step without modifying the loading interception logic.
...
{\setstretch{1}%
\begin{longtable}{|p{0.06\textwidth}|p{0.12\textwidth}|p{0.19\textwidth}|p{0.08\textwidth}|p{0.42\textwidth}|}
...
A static \texttt{const char*} array named \texttt{ENCRYPTED\_LIBS} in \texttt{nativecallhook.cpp} lists the protected filenames.  In the prototype, the sole entry is \texttt{"liblibb.so"}.  Hardcoding simplifies the proof of concept but means that each change requires recompilation (a limitation discussed in \cref{sec:routing_limits}). Adding a new library to the protected set requires exactly two source-level changes, satisfying NFR4: a new entry in the \texttt{ENCRYPTED\_LIBS} array in \texttt{nativecallhook.cpp}, and a corresponding \texttt{add\_custom\_command} block in the application's \texttt{CMakeLists.txt}.
...
This test examines whether the ordering of the two calls in the static initialization block (\texttt{NativeCallHook.initialize()} and \texttt{System.loadLibrary("liba")}) affects correct operation. A test build was prepared with the static block reordered so that \texttt{System.\allowbreak loadLibrary("liba")} ran before \texttt{NativeCallHook.\allowbreak initialize()}.
